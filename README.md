...
<p align="center">
  ...
  <a href="#plug-in-your-gate-your-optimizer">Plug in</a> ·
  <a href="#quick-start">Quick start</a> ·
  <a href="#status">Status</a> ·
  <a href="#roadmap">Roadmap</a> ·
  <a href="#packages-and-documentation">Docs</a>
</p>

<br>

samsara is the substrate for recursive self-improvement of agent harnesses, and
the bench on which self-improvement is judged.

**Every change to the harness is a challenger.** It is evaluated in a disposable
scope, scored only by truth from outside the loop, promoted only through a
statistical gate, and adopted only after a human signs off through a channel
the loop cannot reach.

Any optimizer can be the proposer and any published accept rule can be the gate
— in any language — and all of them leave the same evidence on one ledger.

It runs on [DeepSeek Harness](https://github.com/deepseek-ai/deepseek-harness)
(dsh) as a bundle and two profiles: **`host`**, a CLI for scripts and CI, and
**`workbench`**, a conversation with an operator agent that runs the experiments
for you. What changes is the harness, not the weights — skills, prompt
sections, memory, tool configuration. Evaluation artifacts and the three fixed
points are never surfaces.
...
every tier, the measured noise floor, the gate and its parameters, cost, holdout
spend, settlements, append-only re-scores. A coordinate tuple is its spine, so
*comparable* is a rule the gate checks, not a convention the runner keeps.
...
<p align="center">
  ...
  <br>
  <sub>coding-tasks, closed-book: 83 tasks × 3 same-configuration reruns. Rates are bootstrap acceptance rates on one pack, not population error rates.</sub>
</p>
...
<p align="center">
  ...
  <br>
  <sub>propose → run → judge → keep-or-drop → settle. Truth, verdicts and consent flow in; nothing flows the other way.</sub>
</p>
...
Every verdict, consent and settlement lands on the ledger; `serve` renders it read-only.
...
An operator agent holds the `samsara_*` tools and runs calibrations, rounds, campaigns and controls as jobs. Each quotes its cost and waits for an Allow.

`/samsara …` commands are the only place pre-registration and consent happen.
...
<p align="center">
  ...
  <br>
  <sub>The experiment page: hypothesis, budget, rounds with one column per shadow gate, the lineage curve. Every page has a <code>.json</code> twin naming the rows its numbers came from.</sub>
</p>
...
keys, the noise floor, the policy, the round's `k`/`index`, a seed — and writes
a verdict with the full `Compare` on stdout.
...
A **proposer** reads a view directory — the champion's skill, held-in tasks and
outcomes, held-out aggregates, `environment.md` — and writes `proposal.json`
plus a patch.

```python
# a proposer — sdk/py (TypeScript: @oldbulb/samsara-proposer-sdk)
...
cd "$(npm root -g)/@deepseek-ai/dsh" \
    && npm i @deepseek-ai/dsh-storage-sqlite@0.1.1-rc.2      # the ledger's sqlite, into the CLI
...
Then the loop, closed at zero cost on the synthetic pack: a noise floor, a
...
```sh
# the signer's key; only the public half goes to the host
...
# the noise floor, the claim, the control
...
# the campaign: rounds until a promotion, which waits for the signature
...
<details>
<summary><b>More</b> — the model-backed loop, the real pack, the full command list</summary>
<br>
...
off by default: it needs the proprietary `@anthropic-ai/claude-agent-sdk`, an
optional peer. `packs/coding-tasks/runtime/provision.sh` installs the pack's
...
dsh --profile host gate change keep-better@0.1.0 --wait 600     # a gate needs a signed consent before it can promote
...
control; both model-backed loops run the coding pack; the gate is calibrated on
a measured noise floor.

> **The calibration, with its limits.** coding-tasks, closed-book, 83 tasks × 3
> reruns: `gate-default` promotes a rerun of the same configuration 6% of the
> time against a declared 5%, and 0 of 6 real rerun pairs. keep-better promotes
...
<sub>`kernel` (the only dsh importer) · `pack` `book` · `loops` `loops-dsh` `loops-claude-code` `environments` · `workdir` `submit` `sandbox` `scope` · `gate` `gate-catalog` `ledger` `lifecycle` · `champion` `signoff` · `proposers` `proposer-sdk` (+ `sdk/py`) · `runner` `ui` `workbench` · `examples/`</sub>

| | |
|:---|:---|
| [`philosophy.md`](docs/design/philosophy.md) | why the three fixed points sit outside the loop, and what is deliberately not claimed |
| [`architecture.md`](docs/design/architecture.md) | seams, surfaces, coordinates and comparability, the ledger model, the lifecycle, E1–E8 / S1–S8 |
| [`workbench.md`](docs/design/workbench.md) | the two profiles, the operator agent, tools, commands, the consent/approval split |
| [`gate.md`](docs/design/gate.md) · [`loops.md`](docs/design/loops.md) · [`proposers.md`](docs/design/proposers.md) · [`packs.md`](docs/design/packs.md) | the seams in detail |
| [`gate-catalog`](packages/gate-catalog/README.md) | the thirteen accept rules, their sources, and what each one claims |
| [`dsh-plugin-notes.md`](docs/dsh-plugin-notes.md) | writing dsh plugins: the model, the pitfalls, the patterns (Chinese) |
