...
4. **MDE (S1).**
   ...
   $$\mathrm{mde} = \left(z_{1-\alpha/2} + z_{1-\beta}\right)\,\frac{\sigma_{\text{paired}}}{\sqrt{n_{\text{eff}} \cdot R}}$$

   $\sigma_{\text{paired}}$ from the noise floor (one paired sample per task), not from the comparison itself; $R$ = `replicates`, the paired samples per task in this comparison, so repeats buy power; $n_{\text{eff}}$ = distinct entities with paired data. At $\alpha = 0.05$, $\beta = 0.2$ the multiplier is $1.960 + 0.842 = 2.80$. The MDE is a power quantity and binds only in rule 3.
5. **Screen (S4).** held-in $\bar\Delta \ge 0$; on `holdin`, with $z = \bar\Delta \big/ \left(\sigma_{\text{entity}}/\sqrt{n_{\text{eff}}}\right)$, if $z < z_{\text{stop}}$ (`futility.zStop`, default $-1$) ⇒ `drop` (futility-only early stop). Nothing else stops early.
6. **Cost (S8).** $\text{costRatio} = \overline{c_{\text{challenger}}} \big/ \overline{c_{\text{champion}}}$; if $\text{costRatio} > \text{maxRatio}$ and the challenger does not dominate on the (quality, cost) front ⇒ `drop`.
...
8. **Ladder exposure (S7).** What the proposer may see of the holdout is only `ladder.beatBest` — $\bar\Delta > \text{bestSoFar} + \sigma/\sqrt{n}$ — and the rounded best-so-far; raw holdout means stay judge-side.
