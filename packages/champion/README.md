...
- `promote(challengerId, consentId)` requires `ledger.challenger(id).verdict.value === 'promote'` (`NOT_PROMOTABLE`), a `verdict.by` that names the promotion gate — the policy mounted on `ctx.gate`, which must itself be the shipped `gate-default@version` — or a gate with a `gate_change` consent on the ledger under its `name@version` (`FOREIGN_GATE`), and a consents row with that id, `action: 'promote'`, on that challenger (`NO_CONSENT`). A `skill` challenger's `patch.skill_ref` directory is copied to `<skillStore>/<skill_sha>/` (tmp + rename, skipped when present) and becomes `skill_ref`; any other surface's `patch.cordis` rows are appended. The file is written tmp + rename, then `dump()` recomposes the profile (`loadProfile` + `renderConfigDump` from the kernel) and `verifyHotApply` compares the expected and observed shas over every coordinate the kept rows touch; on mismatch the previous file is restored and `HOT_APPLY_MISMATCH` carries both shas. Idempotent on an already-kept challenger. The ledger row's `decided` status with `verdict.consent_id` is the lifecycle's write, after this returns.
- `demote(challengerId)` removes its kept entry (`NOT_KEPT` otherwise), rewrites and re-verifies the file; the `reversed` verdict (`by: 'champion'`, `rule: demote:<reason>`) is the lifecycle's write.
...
- `compareRowOf(challengerId, gateJudgement, {vs_id, tier, truth_snapshot_id}, kept)` (pure) is the compare row a re-score yields: for a kept challenger `promote` becomes `confirmed`, anything else `reversed`; an unkept row keeps the gate's value. `lifecycle.rescore` records it and demotes on a reversal.
