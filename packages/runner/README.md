...
| `samsara-runner` | `@oldbulb/samsara-runner` | `samsaraRun`, `loops`, `agentDefaultModel` | waits for the loader, runs the set through a bounded pool (`--parallel`), prints a summary table, exits through `ctx.appExit`; SIGINT cancels in-flight attempts and still writes their rows |
